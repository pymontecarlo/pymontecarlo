...
The interface was designed to have common input and output that are independent 
of any Monte Carlo code. 
...
The analysis of the results is also simplified by the common output format where
results are expressed in the same units.

**pyMonteCarlo** is mainly designed and developed to fulfill simulation needs 
and solve problems faced by the electron microscopy and microanalysis community.
Adapting **pyMonteCarlo** to other scientific fields is not completely excluded,
but it is a more long-term objective.
...
**pyMonteCarlo** has the following goals:
    ...
    * Provide a common interface to setup Monte Carlo simulations for the
      electron microscopy and microanalysis community.
