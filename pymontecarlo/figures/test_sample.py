...
MATERIAL_DS = Material('Ds', {110: 1.}, 1.)
MATERIAL_RG = Material('Rg', {111: 1.}, 1.)
MATERIAL_AU = Material('Au', {79: 1.}, 1.)

LAYERS = [Layer(MATERIAL_DS, .1),
          Layer(MATERIAL_RG, .15),
          Layer(MATERIAL_AU, .2),
          Layer(MATERIAL_DS, .05)]

def test_samplefigure_draw_nothing(samplefigure, ax):
    samplefigure.draw(ax)
    assert len(ax.collections) == 0

@pytest.mark.parametrize('sample,perspective,expected_path_count',
...
def test_samplefigure_draw_sample(samplefigure, ax, sample, perspective, expected_path_count):
...
@pytest.mark.parametrize('beam,perspective,expected_path_count',
