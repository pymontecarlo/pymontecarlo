...
from pymontecarlo.formats.document.options.sample.sphere import SphereSampleDocumentHandler
from pymontecarlo.options.sample.sphere import SphereSample
from pymontecarlo.options.material import Material

# Globals and constants variables.

class TestSphereSampleDocumentHandler(TestCase):

    def testconvert(self):
        handler = SphereSampleDocumentHandler()
        sample = SphereSample(Material.pure(29), 50e-9, 0.1, 0.2)
        document = self.convert_documenthandler(handler, sample)
        self.assertEqual(6, self.count_document_nodes(document))
