...
class PhotonDetectorSeriesHandler(DetectorSeriesHandler):
    ...
    def convert(self, detector):
        ...
        column = self._create_column('photon detector elevation angle', 'ph. det. theta', 'rad', PhotonDetector.ELEVATION_TOLERANCE_rad)
        s[column] = detector.elevation_rad

        column = self._create_column('photon detector azimuth angle', 'ph. det. phi', 'rad', PhotonDetector.AZIMUTH_TOLERANCE_rad)
