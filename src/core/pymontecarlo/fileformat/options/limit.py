...
from pymontecarlo.options.limit import TimeLimit, ShowersLimit, UncertaintyLimit, _TransitionLimit

# Globals and constants variables.

class _TransitionLimitXMLHandler(_XMLHandler):

    TAG = '{http://pymontecarlo.sf.net}_transitionLimit'
    CLASS = _TransitionLimit
    ...
    def parse(self, element):
        ...
        return _TransitionLimit(transitions)
...
class UncertaintyLimitXMLHandler(_TransitionLimitXMLHandler):

    TAG = '{http://pymontecarlo.sf.net}uncertaintyLimit'
    CLASS = UncertaintyLimit

    def parse(self, element):
        limit = _TransitionLimitXMLHandler.parse(self, element)
        uncertainty = self._parse_numerical_parameter(element, 'uncertainty')
        return UncertaintyLimit(limit.transition, uncertainty)

    def convert(self, obj):
        element = _TransitionLimitXMLHandler.convert(self, obj)
