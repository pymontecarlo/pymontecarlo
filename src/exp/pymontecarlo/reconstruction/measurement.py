...
class Measurement(object):
    ...
    def _save(self, hdf5parent):
        ...
        hdf5parent.attrs['transitions'] = [unicode(transition) for transition in self._transitions]
        ...
        for transition, results in self._results_std.iteritems():
            hdf5groupchild = hdf5group.create_group(unicode(transition))
        ...
        for transition, material in self._standards_material.iteritems():
            hdf5group.attrs[unicode(transition)] = xmlutil.tostring(material.to_xml(), pretty_print=False)
